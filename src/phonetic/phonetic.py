...
def find_start_con_pronunciation(syllable: Syllable) -> str:
    start_con_sound = ''
    if syllable.start_cons == 'tj':
        ...
        for i in range(0, len(syllable.start_cons)):
            if i == 0 and syllable.prev_syl and syllable.start_cons[i] == syllable.prev_syl.end_cons[-1:]:
                # blokken = bloken
                continue
            ...
            if (
                (start_con_sound[-1:] != retrieved_sound)
                and syllable.prev_syl
...
def find_end_con_pronunciation(syllable: Syllable) -> str:
    end_con_sound: str = ''

    for i in range(0, len(syllable.end_cons)):
        function_name = f'find_end_{syllable.end_cons[i]}_pronunciation'
        retrieved_sound: str = getattr(EndPronunciations, function_name, EndPronunciations.default_end_consonant_replacement)(syllable, i)  # type: ignore
        if end_con_sound[-1:] != retrieved_sound:
            end_con_sound += retrieved_sound
    return end_con_sound


def find_vowel_pronunciation(syllable: Syllable) -> str:
    vowel_sound: str = ''
    if syllable.start_cons == 'q' and syllable.vowels != '' and syllable.vowels[0] == 'u':
        ...
        elif (syllable.vowels + syllable.end_cons) in {'en', 'er'} and syllable.prev_syl and syllable.prev_syl.text != '' and not syllable.next_syl:
            # lopen and loper
            return '0'
    ...
    logbasic.debug(f'Vowel sound for vowel {syllable.vowels}: {vowel_sound}')
    return vowel_sound


def find_open_vowel_pronunciation(syllable: Syllable) -> str:
    if (
        syllable.text
        in {
            'ge',
...
def ending_vowel(vowel: str) -> str:
    switcher = {
        'e': '0',  # blij-e
        'y': 'í',  # sexy
    }
...
def default_phonetic_symbol(dipthong: str) -> str:
    switcher = {
        'aa': 'á',
        'ee': 'é',
        'ie': 'í',
