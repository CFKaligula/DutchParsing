...
* **list[Syllable]** `syllables`, the syllables which the word is made up of.
* **String** `pronunciation`, a string of text in our custom phonetic system.
...
* **String** `start_cons`, the phonetic consonants at the start of the syllable.
* **String** `vowels`, the phonetic vowels making up the vowel sound in the syllable.
* **String** `end_cons`, the phonetic consonants at the end of the syllable.
...
Realize that `start_cons` and `end_cons` can both be empty strings for example in the word *auto* the syllables would be *au* and *to*;  the first consisting of only a `vowels` section and the second syllable having no ending consonants.

## Splitting Words in Syllables

TODO SHOULD PROBABLY EXPLAIN MORE ABOUT WHAT SPLITTING ACTUALLY DOES AND WHAT SYLLABLES ARE

The first challenge for this program is splitting a whole word into syllables. This is a useful feature for example for text editors for automatically hyphenating words at the end of sentences. In our case we want to split the word into syllables to be able to find the pronunciation of the word.

Dutch has its own rules for splitting up words in syllables that do not align with the technical syllables that a linguist would say the word contains.  For example it is said that starting syllables without starting consonants, should not be split, so the word *eten* should not be split like 'e-ten' but should stay 'eten'. For this project, we ignore such rules as we want to parse the words like they should **actually** be pronounced so we can later give the correct pronunciation of the words. Moreover, adapting the current implementation to follow the actual Dutch syllable splitting rules would not be difficult at all. We will now continue the way the actual syllable splitting is implemented.

When we create a `Word` object, it will compute its own syllables when it is instantiated. The function used for this is the `compute_syllables()` function.

We first create 2 variables, `pos` that tracks the index we are at in our word, and `syllable_list` that keeps track of the syllables we've already found. Then the function goes into a while loop that keeps looping until `pos`is outside of our word.

At the start of the `while` loop, we create a `Syllable` then loop over the letters in the word from `pos` to the end of the word. There is a high chance we will break the loop before the end of the word as we break this loop when we find the end of our syllable. Inside this second loop we make 2 variables `current_letter` and `next_letter`, which speak for themselves. We now have a couple of cases to analyze.

There are firstly 2 trivial cases. The current letter is:

* **a dash**, in this case we immediately break as we see a dash as the end of a syllable.
* **not in the alphabet**, in this case we ignore the letter. So *ba3l* will end up as *bal*.

Then we have our 3 main cases left, the current letter is:

1) a consonant
2) a vowel with an accent
3) a vowel without an accent.

### Consonant

In the case of a consonant, we first check if the letter is the last letter of the word **and** a *y*. This is a kind of quick fix so non-compound english loanwords are still split correctly, e.g. 'baby'. If this is the case, we check if we  already found some ending consonants, since that would mean that our y is the vowel in the second syllable. See the word 'lazy', when we start parsing we find the  'l' as a starting consonant then the 'a' as a vowel then the 'z' as an ending consonant then the 'y'. Since we found that 'y' we realized that 'z' is not an ending consonant of 'laz' but actually the starting consonant of the syllable 'zy'  otherwise the word would be split as 'laz-y' instead of 'la-zy'. But to fix this we should actually start looping again from the 'z' so we start with as a new syllable. To do this we call a function `fix_end_cons()` which we give our `index` in the loop but which will also return our `index` as it will fix the index to be set to the 'z' again. After calling that function we of course break out of the loop since we found the end of our syllable, The syllable that will now be added to the syllable list is 'la' and we call the function `get_syllables()` again with the index of the 'z' and the syllable list appended with the syllable `la`. The reason that we used the `fix_end_cons()` function and did not just decrement the index by one, is that in cases with words with multiple ending consonants, finding what consonants belong to what syllable are a bit more complicated. First of all, if the word would have been 'hutsky', we would have wanted the second syllable to start from 's' and thus the index would have been decremented by 2 instead of 1. This is why in the general case we have to call `fix_end_cons()` which will sort this all out and take care of some edge cases. This function will be explained later on in more detail.
...
Now that we have fixed the starting consonants our syllable is officially complete and we append it to the syllable list, then we call the function again with the index that our loop ended with as `start` and the current syllable list as `syllable_list` as parameters to `get_syllables()`. This will eventually lead to the entire word being split into syllables at which point our recursion will end and our syllable list is finished and our word should be successfully and correctly split. Once we have found all syllables, we assign the `next_syl` property for every syllable in the syllable list, as we now know them all, and we return the list.
